/*
  ...
  and a `detail` (what the overlay shows). The card is deliberately thin: the
  long write-ups were a 12k-character wall before this split.

  Bullet strings support **bold** and `code`; see formatBullet in
  ExperienceSection.jsx. Plain strings rather than HTML so nothing has to be
  dangerously injected.
*/
const experience = [
  {
    id: "suny",
  ...
  },
  {
    ...
    current: true,
    stack: ["Python", "PyTorch", "Node.js", "PostgreSQL", "Redis", "WebSockets"],
    headline: [
      { value: "−45%", label: "order-status tickets" },
      { value: "+18%", label: "recall@10 over baseline" },
      { value: "+20%", label: "repeat-order conversion" },
    ],
    blurb:
      "Live delivery tracking and the customer order recommendation system.",
    detail: [
      {
  ...
  },
  {
    ...
    stack: ["Node.js", "PostgreSQL", "Redis", "Angular", "React Native", "AWS"],
    headline: [
      { value: "4,000+", label: "orders a day" },
      { value: "−97%", label: "p99 latency" },
      { value: "2d→20m", label: "reconciliation" },
    ],
    blurb:
      "Core order service, vendor payouts, and the outage that took out a 20x holiday traffic spike.",
    detail: [
      {
        title: "At a glance",
        bullets: [
      ...
      },
      {
        ...
        bullets: [
          "Cut server delivery costs **20%** by moving static asset hosting to S3 and CloudFront with automated deployment scripts.",
          "Replaced manual FTP deploys with a one-command pipeline anyone on the team could run: build, `aws s3 sync`, then a CloudFront invalidation.",
        ],
      },
      {
        title: "Mobile",
        bullets: [
          ...
          "Integrated Firebase Cloud Messaging for driver alerts with custom notification channels, and fixed a ProGuard bug where obfuscated class names broke the FCM SDK at runtime.",
          "Reduced the production APK from 45MB to ~34MB via ProGuard and asset optimization.",
          "Added delivery tracking maps on OpenStreetMap and Leaflet inside a WebView at roughly zero cost, with Supercluster handling 50+ pin clustering.",
        ],
      },
    ],
  },
